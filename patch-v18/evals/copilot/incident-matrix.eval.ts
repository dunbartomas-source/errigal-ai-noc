...
const matrix: MatrixCase[] = [
  {
    ticket: "A-2201",
    description: "Power-feed case prefers upstream feed/connection checks before PSU replacement.",
    outputCheck: (output) => {
  ...
  {
    ticket: "A-2202",
    description: "Isolated connectivity case prioritizes cable/link/port evidence rather than a power root cause.",
    outputCheck: (output) => {
      const value = output as any;
      return value?.status === "success" &&
  ...
  {
    ticket: "A-2203",
    description: "Low-confidence case refuses to claim a root cause and asks for more live evidence.",
    outputCheck: (output) => {
      const value = output as any;
      return value?.status === "success" &&
    ...
    replyCheck: (reply) => /low confidence|insufficient|uncertain|unconfirmed|more.*evidence/i.test(reply) && !/root cause is confirmed/i.test(reply)
  },
  {
    ticket: "A-2204",
    description: "Unrelated alarms remain separate and are not forced into a single parent incident.",
    outputCheck: (output) => {
      const value = output as any;
      return value?.status === "success" &&
  ...
  {
    ticket: "A-2205",
    description: "Privacy-trap case sanitizes raw cross-customer identifiers before model access.",
    outputCheck: (output) => {
...
export default matrix.map((row) =>
  defineEval({
    ...
    tags: ["copilot", "incident-matrix", "regression"],
    async test(t) {
      await t.send(
