...
import { getCopilotIncidentCase } from "../data/copilot-source";
...
export default defineTool({
  ...
  async execute(input) {
    const sourceResult = await getCopilotIncidentCase(input.tenant_id, input.ticket_id);

    if (sourceResult.status !== "success") {
      audit("get_copilot_incident_source", "tenant", sourceResult.source, 0, {
        warningCount: sourceResult.warnings.length
      });
      return {
        status: sourceResult.status,
        data_as_of: DATA_AS_OF,
        is_live: sourceResult.is_live,
        source_records: 0,
        data_source: {
          id: sourceResult.source,
          mode: "read_only",
          configured: sourceResult.status !== "source_unavailable"
        },
    ...
    audit("get_copilot_incident_evidence_pack", "tenant", `${sourceResult.source}_copilot_incident_evidence_pack`, sourceRecords, {
    ...
    return {
      ...
      warnings: [...sourceResult.warnings, ...selected.warnings],
