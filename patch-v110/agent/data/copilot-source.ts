...
export async function getCopilotIncidentCase(tenantId: string, ticketId: string): Promise<CopilotSourceResult> {
  ...
  if (source === "keystats") {
    // Deliberately fail closed until a real read-only KeyStats/NOVA adapter is configured.
    // Never fall back to synthetic data when production data was explicitly requested.
    return {
