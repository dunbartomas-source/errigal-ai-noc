...
function serviceUrl(identifier: string): string | null {
  const configured = String(process.env.AI_NOC_DATA_SERVICE_URL ?? "").trim().replace(/\/$/, "");
  let base = configured.includes("errigal-ai-noc-data-service.vercel.app") ? configured : "https://errigal-ai-noc-data-service.vercel.app/api";
...
async function getKeystatsCase(identifier: string): Promise<CopilotSourceResult> {
  ...
  try {
    const headers = { authorization: process.env.AI_NOC_DATA_SERVICE_TOKEN ? `Bearer ${process.env.AI_NOC_DATA_SERVICE_TOKEN}` : "", "x-vercel-protection-bypass": process.env.VERCEL_AUTOMATION_BYPASS_SECRET ?? "" };
    let response = await fetch(url, { headers, cache: "no-store", redirect: "manual" });
    if (response.status === 404 && /^\\d+$/.test(identifier.trim())) {
      const retryUrl = serviceUrl(`${identifier.trim()}.0`);
      if (retryUrl) response = await fetch(retryUrl, { headers, cache: "no-store", redirect: "manual" });
    }
